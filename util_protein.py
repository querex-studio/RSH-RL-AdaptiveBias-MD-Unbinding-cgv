# util_protein.py — Plotting, metrics, coverage, checkpoints (updated)
import os
import json
import csv
import math
...
# ======================== KPIs & TRAJ ========================
def plot_distance_trajectory(episode_trajectories, episode_num: int, distance_history=None):
    plot_dir = _ensure(f"{config.RESULTS_DIR}/full_trajectories/")
    # fallback
    if (not episode_trajectories or len(episode_trajectories) == 0) and distance_history and len(distance_history) > 1:
    ...
    if full_traj.ndim == 0: full_traj = full_traj.reshape(1)
    ...
    plt.axhspan(config.TARGET_MIN, config.TARGET_MAX, color='green', alpha=0.15, label='Target zone')
    plt.axhline(config.TARGET_CENTER, linestyle='--', linewidth=1.0, label='Target 8.5 Å')
    plt.xlabel("Frames"); plt.ylabel("P–Mg distance (Å)")
    plt.title(f"Episode {episode_num:04d} trajectory")
    plt.legend()
    out_png = os.path.join(plot_dir, f"progressive_traj_ep_{episode_num:04d}.png")
    plt.tight_layout(); plt.savefig(out_png, dpi=200); plt.close()
    print(f"Saved trajectory plot: {out_png}")

    # CSV export
    out_csv = os.path.join(plot_dir, f"progressive_traj_ep_{episode_num:04d}.csv")
    np.savetxt(out_csv, np.c_[time_axis, full_traj], delimiter=",",
...
def plot_bias_components_and_sum(bias_log, backstops_A, episode_num):
    ...
    plt.close(); print(f"Saved bias components plot: {out}")
...
def plot_bias_sum_only(bias_log, backstops_A, episode_num):
    ...
    plt.savefig(out, dpi=300, bbox_inches='tight')
    plt.close(); print(f"Saved cumulative bias plot: {out}")
...
def plot_bias_timeline(bias_log, backstop_events, episode_num):
    ...
    plt.figure(figsize=(10,5)); plt.plot(steps, amps, marker='o')
    plt.xlabel('Bias index (episode step)'); plt.ylabel('Amplitude (kcal/mol)')
    plt.title(f'Bias Amplitude Timeline — Episode {episode_num}')
    plt.grid(True, alpha=0.3)
    out1 = os.path.join(tl_dir, f"timeline_amp_ep_{episode_num:04d}.png")
    plt.savefig(out1, dpi=300, bbox_inches='tight'); plt.close()

    plt.figure(figsize=(10,5)); plt.plot(steps, cents, marker='o')
    for (s, mA) in backstop_events:
        plt.axhline(y=mA, linestyle=':', alpha=0.5)
    plt.axhspan(config.TARGET_MIN, config.TARGET_MAX, alpha=0.18, label='Target Zone')
    plt.axhline(y=config.TARGET_CENTER, linestyle='--', linewidth=2, label='Target Center')
    plt.xlabel('Bias index (episode step)'); plt.ylabel('Center (Å)')
    plt.title(f'Bias Center Timeline — Episode {episode_num}')
    plt.legend(loc='best'); plt.grid(True, alpha=0.3)
    out2 = os.path.join(tl_dir, f"timeline_center_ep_{episode_num:04d}.png")
    plt.savefig(out2, dpi=300, bbox_inches='tight'); plt.close()

    plt.figure(figsize=(10,5)); plt.plot(steps, widths, marker='o')
    plt.xlabel('Bias index (episode step)'); plt.ylabel('Width (Å)')
    plt.title(f'Bias Width Timeline — Episode {episode_num}')
    plt.grid(True, alpha=0.3)
    out3 = os.path.join(tl_dir, f"timeline_width_ep_{episode_num:04d}.png")
...
def plot_lock_snapshot(backstops_A, episode_num):
    ...
    plt.xlabel('Position (Å)'); plt.title(f'Milestone Locks Snapshot — Episode {episode_num}')
    plt.legend(bbox_to_anchor=(1.04, 1), loc='upper left'); plt.tight_layout()
...
def export_episode_metadata(episode_num, bias_log, backstops_A, backstop_events):
    ...
    meta = {
        ...
        'backstop_events': [list(map(float, x)) for x in backstop_events],
