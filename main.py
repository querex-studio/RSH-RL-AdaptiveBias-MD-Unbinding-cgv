# main.py — training loop with warm-up and robust plotting (final)
import os
import numpy as np
import torch
import config
...
def train_progressive(n_episodes=400):
    ...
    for ep_idx in range(1, n_episodes + 1):
        ...
        state = env.reset(seed_from_max_A=None, carry_state=False)
        done = False
        steps = 0
        ...
        if not getattr(env, "episode_trajectory_segments", []):
            if len(getattr(env, "distance_history", [])) > 1:
                env.episode_trajectory_segments = [list(map(float, env.distance_history[1:]))]

        # plot trajectory
        if getattr(env, "episode_trajectory_segments", []):
            plot_distance_trajectory(env.episode_trajectory_segments, ep_idx,
                                     distance_history=getattr(env, "distance_history", None))
        else:
            print(f"[Warning] No trajectory data recorded for episode {ep_idx}. Nothing to plot.")

        # simple console log
        if metrics:
            print(f"[ep {ep_idx:04d}] steps={steps} "
