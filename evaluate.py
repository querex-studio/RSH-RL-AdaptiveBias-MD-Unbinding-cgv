...
def _latest_checkpoint(ckpt_dir):
    ...
    def epnum(p):
        m = re.search(r"ckpt_ep_(\d+)\.pt$", os.path.basename(p)); return int(m.group(1)) if m else -1
    files.sort(key=epnum); return files[-1]
...
def one_episode_eval(env, agent, max_steps):
    traj = []; ret = 0.0; steps = 0; done = False
    s = env.get_state()
...
def evaluate_policy(agent, starts_A, episodes_per_start=10, max_steps=None):
    ...
    try:
        ...
        rows = []; succ_final = {s: 0 for s in starts_A}; succ_any = {s: 0 for s in starts_A}
        for sA in starts_A:
            for ep in range(episodes_per_start):
                ...
                env.distance_history.clear(); env.distance_history.append(float(sA))
...
def main():
    ...
    with open(out_csv, "a", newline="") as f:
        w = csv.DictWriter(f, fieldnames=['start_A','episode_idx','steps','reward','end_A','success_final','success_anytime'])
